...
const weights = {
  ...
  placePercentage: 0.539, earningsPerStart: 1.829, genderAdjustment: 0,
  trackFamiliarity: 0, consistencyFactor: 1.489, driverPerformance: 0.436,
  distanceAdjustment: 1.105, horseWinPercentage: 1.148, trainerPerformance: 0,
...
async function getCachedRawTimes(raceId: string) {
  const resp = await fetch(`${API_BASE}/api/rawtimes/${raceId}`);
...
async function main() {
  ...
  for (const dateEntry of dataset as any[]) {
    ...
    for (const race of dateEntry.races) {
      const cached = await getCachedRawTimes(race.raceId);
      if (!cached) {
        cacheMisses++;
        races.push({ ...race, rawKmTimes: [] });
        continue;
      }
      cacheHits++;
      races.push({
        ...race,
        rawKmTimes: cached.rawTimes.map((c: any) => ({
          horseKey: c.horseKey,
          horseId: c.horseId,
    ...
    mapped.push({ ...dateEntry, races });
  }

  const result = await evaluateWeights(mapped as any, weights as any, postPositionCurves as any);
