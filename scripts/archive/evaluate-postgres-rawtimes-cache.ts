...
function loadRawTimes(path: string) {
  const rows = JSON.parse(fs.readFileSync(path, 'utf8'));
  const out = new Map<string, any[]>();
  for (const row of rows) out.set(row.raceId, row.rawTimes ?? []);
  return out;
}

async function main() {
  const datasetPath = process.argv[2] ?? 'calibration-dataset-6mo_lat_5_11_3.json';
  const rawTimesPath = process.argv[3] ?? 'reports/postgres-raw-times-2026-05-11.json';
  const dataset = loadDataset(datasetPath);
  const rawTimesByRace = loadRawTimes(rawTimesPath);

  let matchedRaces = 0;
  let injectedRawTimes = 0;
  const replayDataset = dataset.map((dateEntry: any) => ({
    ...dateEntry,
    races: (dateEntry.races ?? []).map((race: any) => {
      const cached = rawTimesByRace.get(race.raceId);
      if (!cached) return race;
      matchedRaces++;
      injectedRawTimes += cached.length;
      return {
        ...race,
        rawKmTimes: cached.map((c: any) => ({
          horseKey: c.horseKey,
          horseId: c.horseId,
  ...
  const result = await evaluateWeights(replayDataset as any, weights as any, postPositionCurves as any);
